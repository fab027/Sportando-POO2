...
CREATE TABLE public.profiles (
  ...
  role public.app_role NOT NULL DEFAULT 'apostador',
...
AS $$
BEGIN
  INSERT INTO public.profiles (user_id, nome, sport_profile, role)
  VALUES (
    NEW.id,
    COALESCE(NEW.raw_user_meta_data->>'nome', split_part(NEW.email, '@', 1)),
    COALESCE((NEW.raw_user_meta_data->>'sport_profile')::public.sport_profile, 'futebol'::public.sport_profile),
    COALESCE((NEW.raw_user_meta_data->>'role')::public.app_role, 'apostador'::public.app_role)
  );
  RETURN NEW;
END;
$$;

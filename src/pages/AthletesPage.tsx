...
const PlayerCard = ({
  ...
  const esporte: "football" | "basketball" =
                ...
                {player.seasons.map((s, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      {s.rating > 0 ? (
                        <span
                          className={`inline-flex items-center justify-center rounded-md px-2 py-0.5 text-xs font-bold ${getRatingColor(s.rating)}`}
                        >
                          {s.rating.toFixed(2)}
                        </span>
                      ) : (
...
const AthletesPage = () => {
  ...
  const [selectedPlayerUrl, setSelectedPlayerUrl] = useState<string | null>(
    null,
  );
  ...
  const { data: playerData, status: playerStatus } =
    usePlayerStats(selectedPlayerUrl);
  const { data: standings } = useStandings(league.sofascoreUrl);
  const { data: teamPlayers, status: teamPlayersStatus } =
    useTeamPlayers(selectedTeam);
  ...
  const filteredTeamPlayers = useMemo(() => {
    return teamPlayers.filter((p) => {
      if (
        ...
        positionGroup(p.position) !== filters.position
      )
        return false;
      if (filters.age !== "all" && ageBucket(p.age) !== filters.age)
        return false;
      if (
        filters.nationality !== "all" &&
        p.nationality !== filters.nationality
      )
        return false;
      return true;
    });
  ...
  if (selectedPlayerUrl && playerStatus === "loading") {
    return (
      <div className="space-y-6">
        <button
          onClick={() => setSelectedPlayerUrl(null)}
          className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
        >
          <ArrowLeft className="h-4 w-4" /> Voltar
        </button>
    ...
    <div className="space-y-6">
      ...
      {mode === "search" && (
        <div className="space-y-4">
                    ...
                    <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
                      ...
                      {!r.team && !r.age && (
                        <span className="line-clamp-1">
                          {r.description || "Clique para ver detalhes"}
                        </span>
                      )}
                    </div>
                  </div>
          ...
          {search.length < 3 && (
            <p className="text-sm text-muted-foreground py-4">
              Digite pelo menos 3 caracteres para buscar um jogador no
              SofaScore.
            </p>
          )}
        </div>
      ...
      {mode === "team" && !selectedTeam && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Selecione uma equipe para ver o elenco profissional masculino:
          </p>
          {standings.length === 0 ? (
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <RefreshCw className="h-4 w-4 animate-spin" /> Carregando equipes
              de {league.name}...
            </div>
          ) : (
      ...
      {mode === "team" && selectedTeam && (
        <div className="space-y-4">
          ...
          <FilterBar
            ...
            onClear={() =>
              setFilters({ position: "all", age: "all", nationality: "all" })
            }
          />

          {teamPlayersStatus === "loading" && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground py-8">
              <RefreshCw className="h-4 w-4 animate-spin" /> Carregando elenco
          ...
          {teamPlayersStatus === "error" && (
            <p className="text-sm text-destructive py-4">
              Erro ao carregar o elenco. Tente novamente.
            </p>
          )}

          {teamPlayersStatus === "success" &&
